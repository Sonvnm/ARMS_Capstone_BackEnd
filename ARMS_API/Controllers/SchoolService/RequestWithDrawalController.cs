...
namespace ARMS_API.Controllers.SchoolService
{
    ...
    public class RequestWithDrawalController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetRequestWithDrawal(string CampusId, string? Search, int CurrentPage, TypeofRequestChangeMajor? Status)
        {
            try
            {
                ...
                // Map to DTO
                result.Item = _mapper.Map<List<RequestWithDrawal_SS_DTO>>(requests);
        ...
        public async Task<IActionResult> AcceptRequestWithdrawal(int RequestID, [FromBody] Reply_Request_DTO Reply_Request_DTO)
        {
            try
            {
